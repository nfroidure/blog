...
import Anchor from "../app/components/a";
import Anchored from "../app/components/anchored";
import Blockquote from "../app/components/blockquote";
import Heading1 from "../app/components/h1";
import Heading2 from "../app/components/h2";
import Heading3 from "../app/components/h3";
import Heading4 from "../app/components/h4";
import Heading5 from "../app/components/h5";
import Heading6 from "../app/components/h6";
import HorizontalRule from "../app/components/hr";
import ListItem from "../app/components/li";
import OrderedList from "../app/components/ol";
import Paragraph from "../app/components/p";
import UnorderedList from "../app/components/ul";
import Strong from "../app/components/strong";
import Emphasis from "../app/components/em";
...
import Gist from "../app/components/gist";
import TootQuote from "../app/components/tootQuote";
import SlidesDotCom from "../app/components/slidesDotCom";
...
const codeMap: NodeToElementMapper<MarkdownCodeNode> = (context, node) => {
  ...
  if (gist) {
    return (
      <Gist key={context.index} username={gist[1]} id={gist[2]}>
        {node.value}
      </Gist>
    );
  }

  return <Code key={context.index}>{node.value}</Code>;
};
...
const blockquoteMap: NodeToElementMapper<MarkdownBlockquoteNode> = (
  ...
  const linkNode =
    ...
    lastNode.children[lastNode.children.length - 1];

  if (linkNode && linkNode.type === "link" && linkNode.title.includes("🐘")) {
    return (
...
const hyperlinkMap: NodeToElementMapper<MarkdownLinkNode> = (context, node) => {
  ...
  return node?.title?.startsWith("🎚") ? (
    <SlidesDotCom key={context.index} url={qualifyPath(node.url)}>
      <Anchor
        href={node.url}
